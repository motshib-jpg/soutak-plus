...
|---|---|---:|---:|---|---|---|
| R01 | Google does not discover/index most new URLs | 4 | 5 | EXTERNAL / MONITORED | XML sitemap + RSS discovery feed, both advertised through robots, canonicals, static HTML, contextual internal links, GSC indexing tracker | Majority of new URLs still unknown/discovered-not-indexed after 2–4 weeks |
...
| R03 | Content is technically correct but too generic/reproducible | 3 | 5 | MITIGATING | Five core pillar pages now include official sources + first-party evidence/templates; permanent policy forbids scaled filler and QA locks the pillar evidence markers | New article has no original example, tool, template, evidence, or defensible added value |
...
| R06 | Weak authorship / trust signals | 2 | 4 | MITIGATED | Visible raw-HTML editorial bylines on all articles, author organization schema, editorial methodology, correction policy, source lists and first-party evidence on factual pillar pages | Readers cannot tell who is responsible for content or how claims were checked |
| R07 | Production change reaches users before validation blocks it | 1 | 5 | MITIGATED | Automatic Vercel deployment from `main` is disabled in `vercel.json`; permanent CI asserts the freeze; production release is manual after validation; immutable deployments preserve rollback | `"main": false` removed, release performed before validation, or manual production release bypasses the policy |
| R08 | Manual static publishing becomes inconsistent at scale | 3 | 4 | MITIGATED | Dynamic filesystem-derived QA, sitemap parity, RSS parity, internal-link checks, minimum quality invariants | Article count grows while publishing remains manual and QA begins requiring exceptions |
| R09 | Orphan or weakly linked articles reduce discovery | 1 | 4 | MITIGATED | Hub links every article; permanent QA requires every article to have a contextual inbound link outside the hub as well as sitemap/RSS presence | CI reports a missing hub/contextual inbound link |
...
| R11 | Shared/generic social presentation reduces external click-through | 3 | 2 | ACCEPTED / IMPROVE OVER TIME | Accurate OG metadata and consistent brand cover today; article-specific visual assets preferred when external sharing becomes material | External sharing becomes a material acquisition channel but CTR is weak |
| R12 | No reliable behavioral measurement | 2 | 4 | MITIGATED | Privacy-minimal first-party page/tool analytics + GSC; GA4 remains an optional additional layer rather than a single point of failure | Page views occur but measurement cannot answer the operational question needed for the next decision |
...
| R22 | Database data loss | 2 | 5 | PARTIAL / EXTERNAL OPERATION | Git source protects code/content; operational DB data requires provider/periodic export backup appropriate to plan once material | Subscriber/request counts change unexpectedly or restore is needed |
| R23 | Canonical/robots/sitemap drift | 1 | 5 | MITIGATED | QA derives expected URLs from filesystem; scheduled live monitor; short crawler-file cache; RSS provides a second discovery surface | Sitemap count differs from public pages, canonical mismatch, robots blocks public paths |
...
| R33 | External sharing has low CTR because visuals are generic | 3 | 2 | OPEN IMPROVEMENT | Pillar-specific inline visual structures and accurate titles; add dedicated social images when distribution data shows the need | Social impressions grow but outbound CTR remains weak |
